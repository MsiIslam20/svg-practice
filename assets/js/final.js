...
const createLights = () => {
  ...
  light2 = new THREE.DirectionalLight(0xe8b297, .02);
	light2.position.set(-600, 350, 350);
  
  light3 = new THREE.DirectionalLight(0xe8b297, .02);
...
const bubbleGeometry = new THREE.SphereGeometry( 180, vertex, vertex );
let bubble;
const createBubble = () => {
...
const updateVertices = (time) => {
  ...
  for(let i = 0; i < bubbleGeometry.vertices.length; i++) {
    ...
    let ratio = ((perlin * 0.18 * (dist + 0.1)) + 0.8);
...
$(".home_inner").on("mousemove" , function(e){
    centerX = $(this).width()/2;
    var moveX = centerX - e.offsetX;
    $(this).css({
        'transform' : 'rotateY('+ moveX/50 +'deg) ',
    })
})